...
**5.1.1** Prepare a study-specific Investigator Site File (ISF) per [SOP-CR-015](/sops/cr-015) and store all essential documents in the ISF in a timely, ongoing fashion throughout the study.

**5.1.2** Ensure all team members delegated clinical data management tasks have valid institutional training (→ [SOP-CR-003](/sops/cr-003)) and have been trained on study documents, source document forms, and CRFs before signing the Task Delegation Log.
...
**5.2.6** Create an Informed Consent Process checklist per [SOP-CR-008](/sops/cr-008) to document compliance of the consent process for each participant and each ICF version signed.
...
- **IP wallet cards** (REB-approved) for participants on IP studies → [SOP-CR-008](/sops/cr-008)
...
- **AE Assessment Log** (→ [SOP-CR-012](/sops/cr-012)) — all AEs from ICF date to end of study; changes from AE to SAE documented here and on the SAE Reporting Log
...
- **External SUSAR Reporting Log** (→ [SOP-CR-012](/sops/cr-012)) — SUSAR receipt date, REB reporting criteria and date
...
- **Protocol/GCP/SOP Deviation Log** and CAPA form(s) for major/critical deviations → [SOP-CR-026](/sops/cr-026)
...
All source document templates are available in Word format on the RI Portal (Clinical Research SOP Appendix section) for building the ISF or TMF per [SOP-CR-015](/sops/cr-015). The MUHC Research Pharmacy maintains its own IP accountability logs → [SOP-CR-010](/sops/cr-010). For medical device accountability → [SOP-CR-024](/sops/cr-024).
