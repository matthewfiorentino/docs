...
This SOP describes the creation of protocol visit-specific source document forms and logs, quality control (by site and Sponsor), and management of clinical study data at the Investigator's/Sponsor-Investigator's site — whether paper or electronic — to ensure that data collection and transcription follows the REB-approved study documents, GCP, ALCOA+ principles, Health Canada regulations, and 21 CFR Part 11 for electronic records (referenced in Health Canada GUI-0100).

This SOP applies to the research community (employees, investigators, physicians, management, consultants, students, volunteers, or other persons) involved in the collection and management of human research data obtained from participants at the MUHC/RI-MUHC.

---

## Responsibilities

The **Sponsor-Investigator or QI/PI** is responsible for ensuring that clinical data management activities at the site meet all applicable Health Canada regulatory, ICH-GCP E6/ISO 14155 GCP, Sponsor, and local requirements.

The **Sponsor/Sponsor-Investigator** is responsible for conducting quality control to ensure all data required by the REB-approved protocol is recorded in source documents using Good Documentation Practices (GDPs), and conducting source data verification to ensure CRF data accurately reflects source documents. → SOP-CR-013
...
**5.1.1** Prepare a study-specific Investigator Site File (ISF) per SOP-CR-015 and store all essential documents in the ISF in a timely, ongoing fashion throughout the study.

**5.1.2** Ensure all team members delegated clinical data management tasks have valid institutional training (→ SOP-CR-003) and have been trained on study documents, source document forms, and CRFs before signing the Task Delegation Log.
...
**5.1.5** Protect and secure study participant data and Sponsor proprietary information. Never share nominative participant information with the external Sponsor (note: sharing with the Sponsor's monitor who has signed an MUHC confidentiality agreement is expected). Keep data in a locked area protected from fire or water damage. Ensure electronic data is filed in a validated system compliant with 21 CFR Part 11 per SOP-CR-015; otherwise use paper.
...
**5.2.5** Create a Training Log (→ SOP-CR-003) and Task Delegation Log (→ SOP-CR-002) as the first logs in the ISF. All other essential document logs are listed in SOP-CR-015.

**5.2.6** Create an Informed Consent Process checklist per SOP-CR-008 to document compliance of the consent process for each participant and each ICF version signed.

**5.2.7** Create and maintain the following participant-specific GCP-required logs (→ SOP-CR-009):
...
- **IP wallet cards** (REB-approved) for participants on IP studies → SOP-CR-008
...
- **AE Assessment Log** (→ SOP-CR-012) — all AEs from ICF date to end of study; changes from AE to SAE documented here and on the SAE Reporting Log
- **Local SAE Reporting Log** (→ SOP-CR-012) — QI/PI SAE awareness date, report date, REB and Health Canada reporting criteria
- **External SUSAR Reporting Log** (→ SOP-CR-012) — SUSAR receipt date, REB reporting criteria and date
...
- **Human Sample Collection, Processing and Shipping Log** (if applicable) → SOP-CR-011
- **Protocol/GCP/SOP Deviation Log** and CAPA form(s) for major/critical deviations → SOP-CR-026
- **Monitoring Visit Log** (if applicable) → SOP-CR-013

Other available logs: Telephone/VC Contact Log, Temperature Log, Confirmation and End of Participation forms.

All source document templates are available in Word format on the RI Portal (Clinical Research SOP Appendix section) for building the ISF or TMF per SOP-CR-015. The MUHC Research Pharmacy maintains its own IP accountability logs → SOP-CR-010. For medical device accountability → SOP-CR-024.
...
**5.3.2** Conduct data management respecting privacy and confidentiality per Quebec Law 5, TCPS2, RI-MUHC institutional policies, the confidentiality agreement, and the Clinical Study Agreement.
...
**5.3.4** → SOP-CR-019 for additional confidentiality and privacy requirements.

### 5.4 Source data collection

**5.4.1** Do not collect any study-specific data until all required approvals have been obtained (Health Canada if applicable, REB, MUHC Authorization, and Sponsor's Go Letter) and the participant has provided informed consent (or REB has provided a waiver of consent).
...
**5.4.6** Per Health Canada GUI-0100, any source document (created by the CRC or printed from OACIS) must be signed and dated by the person collecting, recording, reviewing, and/or assessing the information to ensure traceability.
...
**5.5.2** CRFs (paper or electronic) are created by the Sponsor/Sponsor-Investigator, completed by the CRC (transcribing de-identified source data), and shared with the Sponsor/Sponsor-Investigator. → SOP-CR-100 for CRF design (Sponsor-Investigators). The Sponsor/Sponsor-Investigator must create a CRF Data Entry Manual/guideline for sites.

**5.5.3** Ensure appropriate team members are trained on the CRF/eCRF, with training documented (SIV training log, self-training, or other) and the task delegated on the Task Delegation Log.
...
**5.9.6** The control, verification, and correction process must allow comparison with source data and indicate who, when, and why modifications were made. Delegation to the persons making modifications must be documented on the Task Delegation Log (→ SOP-CR-002).
...
**5.10.2** Per SOP-CR-015, retain all essential study documents for a minimum of 15 years (Health Canada-regulated) or 7 years from publication date (non-regulated).
...
**5.10.4** Protect all paper and/or electronic data from deterioration, accidental damage, or destruction for the full retention period. The RI-MUHC can open an account at Iron Mountain (→ SOP-CR-015).
...
**5.10.6** Ensure all essential documents are accessible and audit/inspection-ready. Regulatory inspectors (Health Canada, FDA) or funding agencies may require access without notice. → SOP-CR-017.

---

## Appendices

- Appendix 1 — Note-to-File (NTF) template (complete electronically or manually)
- Appendix 2 — Medical History Events Log
- Appendix 3 — Medical History Medication Log
- Appendix 4 — Concomitant Medication Log
...
<Update label="SOP-CR-014_08" description="01 May 2025">
  Combination of N2 SOP014_10 and RI-MUHC SOP-CR-014_07 Addendum with clarifications. Addition/revision to 5.1.1, 5.1.3, 5.1.5, 5.2, 5.3.4, 5.4.2, 5.4.4, 5.4.5, 5.5, 5.6.1 (last point), 5.6.2 (3rd point), 5.7.4, 5.8.1, 5.9.1, and 5.10.3. Revision of Appendix 1; addition of Appendices 2, 3, and 4. Added Health Canada GUI-0100 and 21 CFR Part 11 to references.
</Update>

<Update label="SOP-CR-014_07" description="01 Sep 2018">
  Section 1.1.3: specifying use of N/Ap and N/Av. Section 1.1.6: addition of justification requirement for corrections. Section 1.2.1: documentation of QC. Section 1.2.5: addition of Task Delegation Log requirement. Added Definitions section (4.0). Renamed from SOP014_06.
</Update>

<Update label="SOP014_06" description="24 May 2016">
  Combination of N2 SOP014_06 and RI-MUHC SOP-CR-009_EN04.
</Update>
