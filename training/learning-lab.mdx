...
/* — Layout: rail + pane — */
.ll-wrap { display: grid; grid-template-columns: 200px 1fr; gap: 0; }
.ll-rail { padding: 28px 16px 28px 20px; border-right: 1px solid var(--ll-bdr); position: sticky; top: 64px; height: calc(100vh - 64px); overflow-y: auto; }
.ll-pane { padding: 36px 40px 80px; min-width: 0; }

/* — Rail header — */
.ll-rail-home { display: block; width: 100%; text-align: left; background: none; border: none; cursor: pointer; font: inherit; font-size: 17px; font-weight: 700; color: var(--ll-text); letter-spacing: -.01em; padding: 0 0 20px; margin-bottom: 4px; transition: color .15s; }
.ll-rail-home:hover { color: var(--ll-accent); }

/* — Rail mode buttons — */
.ll-rail-nav { display: flex; flex-direction: column; gap: 2px; margin-bottom: 28px; }
.ll-rail-btn { display: block; width: 100%; padding: 9px 12px; border: none; background: none; cursor: pointer; font: inherit; font-size: 13.5px; color: var(--ll-text-2); text-align: left; border-radius: 6px; transition: background .15s, color .15s; }
.ll-rail-btn:hover { background: #f7f7f7; color: var(--ll-text); }
.ll-rail-btn.active { background: var(--ll-accent-soft); color: var(--ll-accent); font-weight: 600; }

/* — Rail footer — */
.ll-rail-footer { font-size: 12px; color: var(--ll-text-3); padding-top: 20px; border-top: 1px solid var(--ll-bdr); }
.ll-rail-footer a { color: var(--ll-text-3); }
.ll-rail-footer a:hover { color: var(--ll-accent); }

/* — Mobile dropdown (hidden on desktop) — */
.ll-mobile-nav { display: none; padding: 16px 20px; border-bottom: 1px solid var(--ll-bdr); background: #fff; position: sticky; top: 64px; z-index: 10; }
.ll-mobile-select-wrap { position: relative; }
.ll-mobile-select-wrap::after { content: "\\25BE"; position: absolute; right: 16px; top: 50%; transform: translateY(-50%); font-size: 12px; color: var(--ll-text-2); pointer-events: none; }
.ll-mobile-select { width: 100%; padding: 10px 14px 10px 14px; padding-right: 38px; font: inherit; font-size: 14px; color: var(--ll-text); background: #fff; border: 1px solid var(--ll-bdr); border-radius: var(--ll-radius); appearance: none; -webkit-appearance: none; cursor: pointer; }
.ll-mobile-select:focus { outline: none; border-color: var(--ll-accent); }
...
@media (max-width: 1024px) {
  ...
  .ll-pane { padding: 28px 20px 80px; }
}

/* — Print — */
@media print {
  .ll-rail, .ll-mobile-nav { display: none !important; }
  .ll-wrap { grid-template-columns: 1fr; }
  .ll-pane { padding: 0; }
}
...
<div class="ll">
  <div class="ll-mobile-nav">
    <div class="ll-mobile-select-wrap">
      <select class="ll-mobile-select" id="ll-mobile-select" aria-label="Learning Lab mode">
        <option value="kc">Knowledge Checks</option>
        <option value="te">Team Exercises</option>
        <option value="dr">Document Review</option>
        <option value="jc">Judgement Calls</option>
        <option value="insp">Inspection Prep</option>
      </select>
    </div>
  </div>
  <div class="ll-wrap">
    <aside class="ll-rail" aria-label="Learning Lab modes">
      <button class="ll-rail-home" data-mode="about" type="button">Learning Lab</button>
      <nav class="ll-rail-nav">
        <button class="ll-rail-btn" data-mode="kc"   type="button">Knowledge Checks</button>
        <button class="ll-rail-btn" data-mode="te"   type="button">Team Exercises</button>
        <button class="ll-rail-btn" data-mode="dr"   type="button">Document Review</button>
        <button class="ll-rail-btn" data-mode="jc"   type="button">Judgement Calls</button>
        <button class="ll-rail-btn" data-mode="insp" type="button">Inspection Prep</button>
      </nav>
      <div class="ll-rail-footer">Progress is not saved.</div>
    </aside>
