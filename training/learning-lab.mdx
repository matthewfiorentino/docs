...
`;
export const LL_JS_2 = `    ref:'<a href="SOP-CR-008-overview.html">SOP-CR-008 \u00a75.3.1</a>' },
...
export const LearningLab = () => {
  useEffect(() => {
    ...
    // Schedule multiple stat-refresh attempts to handle Mintlify's
    // settling re-renders, which can reset dangerouslySetInnerHTML
    // content at unpredictable points after the script loads.
    const timers = [400, 900, 1600].map(delay =>
      setTimeout(() => {
        if (typeof window.llRefreshStats === 'function') window.llRefreshStats();
      }, delay)
    );
  ...
  return (
    <>
      ...
      <div style={{overflowX: 'hidden'}} dangerouslySetInnerHTML={{__html: LL_HTML_1 + LL_HTML_2}} />
    </>
  );
};
