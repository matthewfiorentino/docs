...
export const LL_CSS = `:root {
  --navy:        #2b2666;
  --navy-card:   #2b2666;
  --teal:        var(--ri-accent, #1572a7);
  --teal-bright: var(--ri-hero-accent, #2b9ce2);
  --teal-chip:   #60f6e5;
  --teal-ll:     #e0f7f5;
  --sky:         #2b9ce2;
  --sky-l:       #e5f3fb;
...
.ll {
  --ll-accent: var(--ri-accent, #1572a7);
  --ll-accent-soft: #f0faf9;
  --ll-text: #111;
  --ll-text-2: #555;
  --ll-text-3: #888;
  --ll-bdr: #e5e5e5;
...
/* — Layout: rail + pane — */
.ll-wrap { display: grid; grid-template-columns: 252px 1fr; gap: 0; max-width: 1180px; margin: 0 auto; }
.ll-rail { padding: 36px 24px 36px 32px; border-right: 1px solid var(--ll-bdr); position: sticky; top: 64px; height: calc(100vh - 64px); overflow-y: auto; }
...
export const LearningLab = () => {
  useEffect(() => {
    const existing = document.getElementById('ll-script');
    if (existing) existing.remove();
    const script = document.createElement('script');
    script.id = 'll-script';
    script.src = '/ll-script.js';
    document.body.appendChild(script);
    return () => {
      const s = document.getElementById('ll-script');
      if (s) s.remove();
    };
  }, []);
