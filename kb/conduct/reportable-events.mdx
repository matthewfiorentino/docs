...
The diagram below shows how a single SAE triggers parallel reporting tracks with different clocks. The Health Canada track applies only to Sponsor-Investigator studies — for industry-sponsored studies, the Sponsor handles HC reporting.
...
flowchart TD
    EVENT(["SAE occurs<br/>or external SUSAR received"]) --> ASSESS{"Assess seriousness,<br/>expectedness, relatedness"}

    subgraph SITE["SITE TEAM"]
        ASSESS
        SDOC["Document in source docs<br/>and SAE log"]
    end

    subgraph SPONSOR["TO SPONSOR"]
        SAE24["Notify Sponsor within 24 hours<br/>of becoming aware"]
    end

    subgraph REB_HC["TO REB · TO HEALTH CANADA (S-I only)"]
        REB7["Life-threatening SAE / SUSAR:<br/>F3H within 7 days<br/>+ detailed report at day 8"]
        REB15["Other reportable SAE / SUSAR:<br/>F3H within 15 days"]
        HC7["Sponsor-Investigator only:<br/>CIOMS I to Health Canada<br/>same 7- or 15-day clock"]
    end
    ...
    style EVENT fill:#150e51,color:#fff,stroke:none
    style ASSESS fill:#1a1466,color:#fff,stroke:none
    style SDOC fill:#007a6e,color:#fff,stroke:none
    style SAE24 fill:#ff6b6b,color:#fff,stroke:none
    style REB7 fill:#f59e0b,color:#fff,stroke:none
    style REB15 fill:#007a6e,color:#fff,stroke:none
    style HC7 fill:#f59e0b,color:#fff,stroke:none
```

Color legend: **coral** = 24-hour clock (most urgent), **amber** = 7-day clock, **teal** = 15-day clock.
