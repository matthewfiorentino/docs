...
flowchart TD
    ...
    SUB --> REB[REB Review\nscientific + ethics]
    SUB --> MUHCF[MUHC Feasibility\nReview]
    SUB --> HC[Health Canada\nCTA / ITA Review\ndrug & device only]

    REB --> AUTH[MUHC Authorization\nPersonne mandatée]
    MUHCF --> AUTH
    HC --> AUTH

    AUTH --> SIV[Site Initiation\nVisit]
    SIV --> GO[Sponsor Go\nLetter]
    GO --> FP([First participant\nmay be approached])

    style SUB fill:#150e51,color:#fff,stroke:none
    style HC  fill:#150e51,color:#fff,stroke:none
    style REB fill:#007a6e,color:#fff,stroke:none
    style MUHCF fill:#007a6e,color:#fff,stroke:none
    style AUTH fill:#1a1466,color:#fff,stroke:none
    style SIV  fill:#007a6e,color:#fff,stroke:none
    style GO   fill:#007a6e,color:#fff,stroke:none
    style FP   fill:#00c4b4,color:#fff,stroke:none
```

<Steps>
  <Step title="Health Canada authorization (drug and device studies only)">
    No Objection Letter (NOL) for drug studies, Notice of Authorization (NOA) for natural health products, or Investigational Testing Authorization (ITA) for medical devices. Runs in parallel with Nagano submission. Must be received before MUHC Authorization can be granted.
  </Step>
  <Step title="REB review and institutional feasibility (parallel tracks via Nagano)">
    REB scientific and ethics review runs in parallel with the MUHC institutional feasibility review (QA, pharmacy, nursing, laboratory, contracts, Director of Professional Services). Both must be complete and satisfactory before the Personne mandatée can issue MUHC Authorization.
  </Step>
  <Step title="MUHC Authorization letter">
    Issued by the Personne mandatée once all review streams are satisfactory. The Authorization letter must be filed in the ISF. It is the institution's formal confirmation that the study may proceed at the MUHC. REB approval alone is not sufficient — a study cannot begin without this letter.
  </Step>
  <Step title="Site Initiation Visit (SIV)">
    The Sponsor or Monitor conducts the SIV after MUHC Authorization is confirmed. The SIV provides protocol-specific training to the full research team and results in the Task Delegation Log being signed. No protocol activities — including screening — may begin before the SIV.
  </Step>
...
Before the SIV can be scheduled, several prerequisite documents must be in place in the ISF and TMF: the Health Canada authorization (where applicable), REB approval for the protocol and all participant-facing materials, the executed Clinical Trial Agreement, the signed protocol, the signed Investigator's Brochure, and CVs and licences for all team members who will be delegated tasks.
...
SOP-CR-006 §5.3.8 is explicit: do not consent participants before all four of the following are confirmed in writing and filed in the ISF.

| # | Condition | Why it exists |
|---|---|---|
| 1 | REB written approval | Ethics and scientific review is complete. The REB has confirmed the study is ethical, the ICF is adequate, and participant rights will be protected. |
...
The Investigator Site File must be set up and organized before any study activities begin. It is a live record assembled from the moment the study is authorized — not a filing system built retroactively. The ISF structure is governed by SOP-CR-015, which maps to ICH GCP E6(R3) Appendix C.
...
<Note>
Each document filed in the ISF must carry a clear version number and date. Documents without version control are an inspection finding. The ISF index in SOP-CR-015 Appendix 4 provides the filing structure that must be used at the MUHC.
</Note>
...
Any equipment used to collect study-specific measurements — scales, blood pressure monitors, spirometers, ECG machines, centrifuges, temperature monitoring devices for IP storage — must have current calibration and maintenance records on file before the study begins. This is governed by SOP-CR-025.
