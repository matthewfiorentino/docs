...
Quebec law requires that any medication administered to a patient — including experimental drugs in clinical trials — be dispensed by a pharmacist. This shapes how investigational product management works at the MUHC in ways that differ from other provinces and from generic GCP guidance.
...
At the MUHC, this means the **MUHC Research Pharmacy** is the central point for all investigational drug management. Once a study is initiated, the IP is shipped directly to the MUHC Research Pharmacy, where it is stored and dispensed by pharmacy staff. The QI delegates the operational tasks of IP management to the pharmacy — but the QI retains ultimate responsibility for the IP and for ensuring it is used in accordance with the approved protocol.
...
The division of responsibility between the QI/PI and the MUHC Research Pharmacy is defined in SOP-CR-010 and its RI-MUHC Addendum. The QI delegates specific tasks to pharmacy but cannot delegate accountability.

<CardGroup cols={2}>

<Card title="QI / PI responsibilities" icon="user-doctor">
- Inform and invite the MUHC Pharmacy to the Site Initiation Visit (SIV) for protocol training
- Prescribe the investigational medication (MDs only)
- Ensure pharmacy holds valid Task Delegation Log entries for IP tasks
- Provide pharmacy with the IP Receipt Log and IP Dispensing Log and ensure they are completed
- Ensure IP is used only in accordance with the approved protocol and Investigator's Brochure or Product Monograph
...
Destruction is conducted by the pharmacy per applicable procedures. Documentation is provided to the Sponsor and a copy is filed in the ISF.
</Step>
...
Storage equipment requirements under SOP-CR-025:
...
IP accountability records must be retained for **15 years** for Health Canada-regulated studies, and 7 years for non-regulated studies, in line with SOP-CR-015.
...
The Investigator requests the exception at the time of study submission in Nagano.
</Step>
...
Regulated under Health Canada's Investigational Testing Authorization (ITA) process. Device handling, storage, and accountability follow device-specific procedures. See SOP-CR-024 for the RI-MUHC framework.
</Accordion>
...
If your study involves any of these IP types, contact the MUHC Research Pharmacy and your QA contact at QAClinicalResearch@muhc.mcgill.ca early in the planning process.
...
- SOP-CR-025 — Equipment Calibration and Maintenance
- SOP-CR-016 — Study Close-Out
