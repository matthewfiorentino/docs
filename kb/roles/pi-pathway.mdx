...
The phases below trace the path from initial credentials through archiving. Each maps to sections on this page and in the guides above.
...
Before opening a Nagano submission, confirm site feasibility: access to the target patient population, required facilities and equipment, team capacity, and estimated enrollment timelines. Use the [Study Feasibility Checklist (SOP-CR-004, Appendix 1)](/sops/cr-004#appendices) to document this assessment. If feasibility review fails during institutional review, the Personne mandatée cannot issue MUHC Authorization — and the study cannot activate.
...
<Tip>
  **Adding trainees, volunteers, or observers?** See the [Onboarding overview](/kb/roles/onboarding) for eligibility, agreements, and scope limitations. **Temporary staff** (visiting fellows, co-op students, contractors) follow the same TDL process — specify their end date on the entry. If a team member fails the Competency Assessment, they cannot be added to the TDL until retrained and reassessed.
</Tip>
...
- **Budget and contract requirements differ by funding model.** For industry-sponsored studies, the Sponsor proposes a Clinical Trial Agreement and budget — your responsibility is to ensure the budget covers all institutional costs (pharmacy, lab, nursing, overhead, archiving) and that the Research Agreements Office has reviewed the agreement. For investigator-initiated studies, you build the budget — use the [RI-MUHC Budget Tool](/kb/planning/data-services) and account for all costs including monitoring, data management, and REB billing fees if foundation-funded. Contracts must be fully executed via My Contracts before MUHC Authorization. See [Contracts, Budgets, and Agreements](/kb/planning/budgets-contracts).
...
- **Conflict of interest declarations** are required at initial submission and every annual renewal. If a COI arises mid-study, disclose promptly via Nagano amendment — do not wait for renewal. See [Conflict of Interest](/kb/governance/conflict-of-interest).
- **15-year retention for Division 5 studies (C.05.012)** — the ISF must remain complete, secure, and retrievable. Do not destroy any study documents without written authorization from you and the Sponsor. You retain personal accountability for the archived file even if you change institutions. See [Research Integrity](/kb/governance/research-integrity) for archiving and retention obligations.
...
<CardGroup cols={3}>
  ...
  <Card title="CIM — Clinical Research Infrastructure" icon="flask-vial" href="/cim/is-cim-right">
